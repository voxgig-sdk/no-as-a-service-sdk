...
import (
	...
	omni "github.com/voxgig-sdk/no-as-a-service-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.NoAsAServiceError); is {
				out := map[string]any{
					"name":    "NoAsAServiceError",
...
func omniCtx(arg any, client *sdk.NoAsAServiceSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.NoAsAServiceSDK); is {
			client = live
