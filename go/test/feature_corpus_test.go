...
//
// NewNoAsAServiceSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.NoAsAServiceSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewNoAsAServiceSDK(opts)
}
...
func fcCandidates(client *sdk.NoAsAServiceSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.NoAsAServiceSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.NoAsAServiceSDK, name string) any {
