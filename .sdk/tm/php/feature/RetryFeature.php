...
// NoAsAService SDK retry feature
...
class NoAsAServiceRetryFeature extends NoAsAServiceBaseFeature
{
    ...
    public function init(NoAsAServiceContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (NoAsAServiceContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(NoAsAServiceContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(NoAsAServiceContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
