...
class NoAsAServiceMakeError
{
    public static function call(?NoAsAServiceContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new NoAsAServiceContext([], null);
        }
        $op = $ctx->op ?? new NoAsAServiceOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new NoAsAServiceResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof NoAsAServiceError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new NoAsAServiceError('', $msg, $ctx);
        ...
        if ($err instanceof NoAsAServiceError) {
