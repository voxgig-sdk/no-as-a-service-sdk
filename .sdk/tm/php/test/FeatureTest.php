...
class FtCtrl extends NoAsAServiceControl
{
...
class FtClient
{
    ...
    private NoAsAServiceUtility $utility;

    public function __construct(NoAsAServiceUtility $utility, array $options)
    {
    ...
    public function get_utility(): NoAsAServiceUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (NoAsAServiceContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public NoAsAServiceContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (NoAsAServiceContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = NoAsAServiceConfig::make_feature($name);
    ...
    private static function buildUrl(NoAsAServiceSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof NoAsAServiceError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new NoAsAServiceResponse($fetched);
        }
    ...
    private function fail(NoAsAServiceContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (NoAsAServiceContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
