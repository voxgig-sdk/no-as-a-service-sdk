...
from projectname_sdk import NoAsAServiceSDK
from projectname_sdk.core.spec import NoAsAServiceSpec
from projectname_sdk.core.result import NoAsAServiceResult
from projectname_sdk.core.response import NoAsAServiceResponse
from projectname_sdk.core.operation import NoAsAServiceOperation
from projectname_sdk.core.error import NoAsAServiceError
from projectname_sdk.core import helpers
from projectname_sdk.feature.base_feature import NoAsAServiceBaseFeature
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return NoAsAServiceError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_feature_init_basic(self):
        init_client = NoAsAServiceSDK.test(None, None)
        ...
        class TestInitFeature(NoAsAServiceBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(NoAsAServiceBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_fetcher_blocked_test_mode(self):
        ...
        blocked_client = NoAsAServiceSDK({
            "base": "http://localhost:8080",
            "system": {
                "fetch": mock_fetch,
            },
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = NoAsAServiceSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = NoAsAServiceSDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = NoAsAServiceOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = NoAsAServiceSDK.test(None, setup_opts)
