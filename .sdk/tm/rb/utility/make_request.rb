...
module NoAsAServiceUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = NoAsAServiceResponse.new({})
    ...
    if fetch_err
      ...
      response = NoAsAServiceResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = NoAsAServiceResponse.new(fetched)
    else
