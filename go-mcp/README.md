...
[MCP](https://modelcontextprotocol.io) server exposing the NoAsAService SDK as
two agent tools — `no-as-a-service_list` and `no-as-a-service_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/no-as-a-service-mcp)
make build

# 2. Provide credentials via the environment
export NO_AS_A_SERVICE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user no-as-a-service \
  -- /absolute/path/to/no-as-a-service-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./no-as-a-service-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// no-as-a-service_list: first page of records
{ "entity": "non" }
{ "entity": "non", "query": { } }

// no-as-a-service_load: one record by id
{ "entity": "non", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/no-as-a-service-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user no-as-a-service \
     -- "$PWD"/dist/*/no-as-a-service-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `no-as-a-service_list` and `no-as-a-service_load` tools now appear
   in new sessions. Ask the agent to *"list non using no-as-a-service"*
   and it calls `no-as-a-service_list` with `{"entity":"non"}`.
...
```sh
export NO_AS_A_SERVICE_APIKEY=sk_live_xxx            # API key
export NO_AS_A_SERVICE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `no-as-a-service_list` tool
...
### Call the `no-as-a-service_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `no-as-a-service_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `no-as-a-service_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `NO_AS_A_SERVICE_APIKEY` | API key sent with every request. |
| `NO_AS_A_SERVICE_BASE` | Optional override of the API base URL. |
...
```sh
./no-as-a-service-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"no-as-a-service_load","arguments":{"entity":"non","query":{"id":1}}}}'
```
