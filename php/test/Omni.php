...
//    only - a typed NoAsAServiceContext there would make every ctx
...
final class NoAsAServiceOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new NoAsAServiceContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new NoAsAServiceSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new NoAsAServiceResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new NoAsAServiceError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new NoAsAServiceResponse($clean['response']);
    ...
    public static function err_from_map($m): ?NoAsAServiceError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new NoAsAServiceError(is_string($code) ? $code : '', $msg);
    }
