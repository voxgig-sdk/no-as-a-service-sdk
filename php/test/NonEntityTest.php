...
class NonEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set NOASASERVICE_TEST_NON_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $non_ref01_match_dt0 = [];
        [$non_ref01_data_dt0_loaded, $err] = $non_ref01_ent->load($non_ref01_match_dt0, null);
        $this->assertNull($err);
...
function non_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("NOASASERVICE_TEST_NON_ENTID");
