...
class NonEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new NonEntity($this->_client, $opts);
    }

    /**
     * @param Non|array $args Non data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = NoAsAServiceHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Non|array The current Non data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Non fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = NoAsAServiceHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Non fields).
     */
     ...
     * Load a single Non.
     *
     * @param NonLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed NonLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Non|array The loaded Non as an assoc-array at the
     *   SDK boundary; throws NoAsAServiceError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = NoAsAServiceHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
