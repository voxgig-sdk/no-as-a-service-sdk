...
// NoAsAService SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class NoAsAServiceTestFeature extends NoAsAServiceBaseFeature
{
    ...
    public function init(NoAsAServiceContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (NoAsAServiceContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = NoAsAServiceParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (NoAsAServiceContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(NoAsAServiceContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = NoAsAServiceParam::call($ctx, $k);
