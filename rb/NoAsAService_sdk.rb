...
require_relative 'NoAsAService_types'
...
class NoAsAServiceSDK
  ...
  def initialize(options = {})
    ...
    utility = NoAsAServiceUtility.new
    @_utility = utility
    ...
    feature_opts = NoAsAServiceHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = NoAsAServiceHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, NoAsAServiceFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    NoAsAServiceUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = NoAsAServiceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = NoAsAServiceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = NoAsAServiceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = NoAsAServiceHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = NoAsAServiceError.new(
        "graphql_error", "NoAsAServiceSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = NoAsAServiceSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
