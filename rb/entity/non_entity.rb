...
class NonEntity
  ...
  def make
    opts = @_entopts.dup
    NonEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = NoAsAServiceHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Non, Hash] the current Non data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = NoAsAServiceHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Non fields)
  def match_get
  ...
  # Load a single Non.
  #
  # @param reqmatch [NonLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Non.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Non, Hash] the loaded Non; raises NoAsAServiceError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = NoAsAServiceHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
