...
local NonEntity = {}
NonEntity.__index = NonEntity


function NonEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, NonEntity)
  self._name = "non"
  self._client = client
...
function NonEntity:get_name()
  return self._name
end


function NonEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return NonEntity.new(self._client, opts)
end
...
function NonEntity:mark_deleted()
  self._deleted = true
end


function NonEntity:deleted()
  return true == self._deleted
end


function NonEntity:data_set(args)
  if args ~= nil then
...
function NonEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function NonEntity:match_set(args)
  if args ~= nil then
...
function NonEntity:stream(action, args, callopts)
...
function NonEntity:_run_op(ctx, post_done)
  local utility = self._utility
