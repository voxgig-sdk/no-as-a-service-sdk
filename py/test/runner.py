...
class NoAsAServiceTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            NoAsAServiceTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = NoAsAServiceTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = NoAsAServiceTestRunner.getenv("NO_AS_A_SERVICE_TEST_LIVE")
        override = NoAsAServiceTestRunner.getenv("NO_AS_A_SERVICE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = NoAsAServiceTestRunner.getenv(key)
        ...
        explain = NoAsAServiceTestRunner.getenv("NO_AS_A_SERVICE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["NO_AS_A_SERVICE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if NoAsAServiceTestRunner._test_control is not None:
            return NoAsAServiceTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                NoAsAServiceTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            NoAsAServiceTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = NoAsAServiceTestRunner.load_test_control()
...
def load_env_local():
    NoAsAServiceTestRunner.load_env_local()


def env_override(m):
    return NoAsAServiceTestRunner.env_override(m)


def entity_data(v):
    return NoAsAServiceTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return NoAsAServiceTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return NoAsAServiceTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return NoAsAServiceTestRunner.load_test_control()


def live_client_options():
    return NoAsAServiceTestRunner.live_client_options()


def live_delay_ms():
    return NoAsAServiceTestRunner.live_delay_ms()
