...
class TestNonDirect:
    ...
    def test_should_direct_load_non(self):
        setup = _non_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-non", "live" if setup["live"] else "unit")
        if _skip:
