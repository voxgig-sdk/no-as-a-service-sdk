...
from noasaservice_sdk.noasaservice_types import (
    Non,
    NonLoadMatch,
)
...
class NonEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return NonEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Non:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Non().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
