...
//     (\`new NoAsAServiceSDK(...)\` / \`NoAsAServiceSDK.test(...)\` become
//     \`NoAsAServiceSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'NoAsAServiceSDK'
...
const TEST_SEED = {"entity":{"non":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new NoAsAServiceSDK(...)\` and any
// \`NoAsAServiceSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+NoAsAServiceSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/NoAsAServiceSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
