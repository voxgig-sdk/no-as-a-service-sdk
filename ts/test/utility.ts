...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.NOASASERVICE_TEST_LIVE ||
    'TRUE' === process.env.NOASASERVICE_TEST_OVERRIDE
  ) {
  ...
  m.NOASASERVICE_TEST_EXPLAIN = process.env.NOASASERVICE_TEST_EXPLAIN || m.NOASASERVICE_TEST_EXPLAIN
