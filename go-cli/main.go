// Command NoAsAService-cli is a CLI + REPL for the NoAsAService SDK,
...
const prompt = "no-as-a-service"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "non"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      NO_AS_A_SERVICE_APIKEY (api key), NO_AS_A_SERVICE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: NO_AS_A_SERVICE_APIKEY carries the API key and
	// NO_AS_A_SERVICE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("NO_AS_A_SERVICE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("NO_AS_A_SERVICE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewNoAsAServiceSDK(opts)
