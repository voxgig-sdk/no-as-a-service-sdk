...
boru-driven command-line client **and** interactive REPL for the NoAsAService
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/no-as-a-service-cli)
make build

# 2. See usage (words, entities, env vars)
./no-as-a-service-cli --help

# 3. Provide credentials once, via the environment
export NO_AS_A_SERVICE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./no-as-a-service-cli load 1 non            # {id:1} shorthand
./no-as-a-service-cli load '{id:1}' non       # explicit match map

# 5. Override the API base URL for a single call
NO_AS_A_SERVICE_BASE=https://api.example.com ./no-as-a-service-cli load 1 non

# 6. No arguments -> interactive REPL
./no-as-a-service-cli
no-as-a-service> load 1 non
no-as-a-service> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/no-as-a-service-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/no-as-a-service-cli load 1 non
   ```
...
```sh
./no-as-a-service-cli load 1 non          # scalar shorthand for {id:1}
./no-as-a-service-cli load '{id:1}' non     # explicit match map
```
...
./no-as-a-service-cli load 1 non
```
...
Run with no arguments to open a REPL (prompt `no-as-a-service>`). Each line is
evaluated as its own boru expression:

```text
$ ./no-as-a-service-cli
no-as-a-service> load 1 non
no-as-a-service> /help
no-as-a-service> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `non`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/no-as-a-service-cli`. |
...
(`./no-as-a-service-cli <expr>`) and interactively (the REPL), and expressions compose the
